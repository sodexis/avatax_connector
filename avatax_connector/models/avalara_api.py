...
class AvaTaxService:
    def __init__(self, username, password, url, timeout, enable_log=False):
        self.username = (
            username  # This is the company's Development/Production Account number
        )
    ...
    def my_profile(self):
        ...
        profile.append(
            suds.sax.element.Element("Client", ns=profileNameSpace).setText(CLIENT)
        )
        profile.append(
            suds.sax.element.Element("Adapter", ns=profileNameSpace).setText(ADAPTER)
        )
        hostname = socket.gethostname()
        profile.append(
            suds.sax.element.Element("Machine", ns=profileNameSpace).setText(hostname)
        )
        return profile
        ...
        if result.ResultCode != "Success":
            for w_message in result.Messages.Message:
                if w_message.Severity == "Error":
                    if (
                        w_message._Name == "TaxAddressError"
                        or w_message._Name == "AddressRangeError"
                        or w_message._Name == "AddressUnknownStreetError"
                        or w_message._Name == "AddressNotGeocodedError"
